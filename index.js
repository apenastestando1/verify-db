...
app.get('/oauth/callback', async (req, res) => {
  ...
  try {
    ...
    if (!user.id) return res.status(400).send('Falha ao obter usuário');
    ...
    res.send('<h1>✅ Verificado!</h1><p>Pode fechar essa página.</p>');
  } catch (err) {
...
app.post('/api/add', requireAuth, async (req, res) => {
  ...
  for (const userId of ids) {
    ...
    try {
      ...
      if (member) {
        results.push({ userId, username: user.username, success: false, error: 'Já está no servidor' });
        continue;
      }
